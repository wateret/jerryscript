...
dump_varg_header_for_rewrite (varg_list_type vlt, jsp_operand_t obj)
{
  ...
  switch (vlt)
  {
    ...
    case VARG_CALL_EXPR:
    {
      ...
      {
        dump_triple_address (VM_OP_NATIVE_CALL,
                             jsp_operand_t::make_unknown_operand (),
                             jsp_operand_t::make_idx_const_operand (name_to_native_call_id (obj)),
                             jsp_operand_t::make_unknown_operand ());
      }
      else
      {
...
{
  dump_single_address (VM_OP_VAR_DECL, jsp_operand_t::make_lit_operand (lit_id));
}
