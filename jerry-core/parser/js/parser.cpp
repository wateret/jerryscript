...
static jsp_operand_t parse_argument_list (varg_list_type, jsp_operand_t, uint8_t *, jsp_operand_t *);
...
parse_property_assignment (void)
{
  if (token_is (TOK_NAME))
  {
    ...
    skip_newlines ();
    const jsp_operand_t func = parse_argument_list (VARG_FUNC_EXPR, empty_operand (), NULL, NULL);
...
/** Parse list of identifiers, assigment expressions or properties, splitted by comma.
    ...
    Result tmp. */
static jsp_operand_t
parse_argument_list (varg_list_type vlt, jsp_operand_t obj, uint8_t *args_count, jsp_operand_t *this_arg_p)
{
